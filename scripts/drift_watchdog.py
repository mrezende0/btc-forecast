"""drift_watchdog — detecta data drift entre baseline e janela atual.

PSI (Population Stability Index) por feature + KS-test.
Alerta no Telegram se max PSI > LIMIT.

Baseline: VAL period (2023-01-01 → 2024-12-31, ~4380 bars 4h).
...
BASELINE_START_MS = int(datetime(2023, 1, 1, tzinfo=timezone.utc).timestamp() * 1000)
BASELINE_END_MS = int(datetime(2024, 12, 31, 23, 59, 59, tzinfo=timezone.utc).timestamp() * 1000)
...
def main():
    ...
    # Particiona em baseline (VAL) vs current (últimos 30d)
    df_pd = df.to_pandas()
    df_pd["open_time"] = df_pd["open_time"].astype("int64")
    baseline_mask = (df_pd["open_time"] >= BASELINE_START_MS) & (df_pd["open_time"] <= BASELINE_END_MS)
    current_mask = df_pd["open_time"] >= df_pd["open_time"].iloc[-1] - BARS_LOOKBACK * 4 * 3600 * 1000
    base = df_pd[baseline_mask]
    curr = df_pd[current_mask]
    print(f"  baseline: {len(base):,} bars ({base['open_time'].min()} → {base['open_time'].max()})")
    ...
    # Summary
    max_psi = results_df.filter(pl.col("psi").is_not_null())["psi"].max() or 0.0
    n_critical = results_df.filter(pl.col("psi") > PSI_CRITICAL).height
    n_alert = results_df.filter((pl.col("psi") > PSI_ALERT) & (pl.col("psi") <= PSI_CRITICAL)).height
    n_warn = results_df.filter((pl.col("psi") > PSI_WARN) & (pl.col("psi") <= PSI_ALERT)).height
    print("\n" + "=" * 110)
    ...
    # ---------- Telegram alert
    critical_features = results_df.filter(pl.col("psi") > PSI_CRITICAL).head(5).to_dicts()
    alert_features = results_df.filter(
